...
export function createMeetingPromptTools(deps: CreateMeetingPromptToolsDeps) {
  ...
  function buildDirectReplyPrompt(
    ...
    const prompt = [
      ...
      "Clarification rules (CRITICAL - do NOT be a yes-man):",
      "- 指示が曖昧・情報不足の場合、実行を確認する前に必ず具体的な質問をしてください。",
      "- 不明な点を勝手に仮定せず、必要な情報を質問で聞き出してください。",
      "- 例: 対象範囲、優先度、期限、判断基準が不明な場合は必ず聞く。",
      "- タスクが明確な場合は、理解した内容と次のステップを確認してから着手。",
      "- 事実を確認できない場合は推測せず「確認が必要です」と正直に伝えること。",
